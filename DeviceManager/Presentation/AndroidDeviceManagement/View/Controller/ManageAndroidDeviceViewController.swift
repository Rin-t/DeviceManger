...
final class ManageAndroidDeviceViewController: UIViewController {
    ...
    private let viewModel = ManageAndroidDeviceViewModel(
        useCase: DeviceDataUseCase(
            dataStore: DeviceDataStore(type: .android)
        )
    )
    ...
    private func setupBindings() {
        ...
        collectionView.rx.itemSelected
            .subscribe(onNext: { [weak self] indexPath in
                ...
                let vc = EditDeviceDataViewController(type: .android, selectedIndex: indexPath)
        ...
        addDeviceButton.rx.tap
            .subscribe(onNext: { [weak self] _ in
                let vc = AddNewDeviceDataViewController(type: .android)
                self?.navigationController?.pushViewController(vc, animated: true)
            })
            .disposed(by: disposeBag)
    }
    ...
    private func setupView() {
        ...
        navigationItem.title = "Androidデバイス管理"
    }
